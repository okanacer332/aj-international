...
export default function Page() {
  ...
  if (!ready) {
    // Yüklenirken boş veya basit bir skeleton döndürebiliriz
    return (
  ...
  return (
    <div className="flex h-full flex-col items-center justify-center space-y-2 text-center">
      {/* ÇEVİRİ: Ana Başlık */}
      <h1 className="text-2xl font-semibold">{t('operation.taskManagement.title')}</h1>
      {/* ÇEVİRİ: Açıklama / Yer Tutucu Metin */}
      <p className="text-muted-foreground">{t('operation.taskManagement.placeholder')}</p>
    </div>
  );
}