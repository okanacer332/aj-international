...
import com.ajinternational.ajserver.modules.masterdata.model.ProductionUnitDefinition; // YENİ İMPORT
import com.ajinternational.ajserver.modules.masterdata.repository.MasterProductRepository;
import com.ajinternational.ajserver.modules.masterdata.repository.ProductionUnitDefinitionRepository; // YENİ İMPORT
...
import java.util.function.Function; // YENİ İMPORT
import java.util.stream.Collectors; // YENİ İMPORT
...
public class MasterProductService {
    ...
    public List<MasterProduct> findAllProducts() {
        ...
        for (MasterProduct product : allProducts) {
            if (product.getProductionUnitId() != null) {
                ...
                if (section != null) {
                    ...
                    if (section.getParentProductionUnitId().isPresent()) {
                        ProductionUnitDefinition group = unitMap.get(section.getParentProductionUnitId().get());
                        if (group != null) {
        ...
        logger.info("Tenant '{}' için tüm ürünler (Grup/Bölüm bilgileriyle) çekildi: {} adet.", tenantId, allProducts.size());
        return allProducts;
    }

    /**
     * Güncellendi: Artık hiyerarşiyi değil, yeni alanları (productionUnitId, unitPrice) kaydeder.
     */
    public MasterProduct saveProduct(MasterProduct product) {
        ...
        String finalUnitId = (rawUnitId != null && !rawUnitId.trim().isEmpty() && !rawUnitId.equals("null")) ? rawUnitId.trim() : null;
